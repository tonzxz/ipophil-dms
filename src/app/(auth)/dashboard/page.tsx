...
type DocumentStatus = keyof typeof statusMapping
type DashboardStatus = 'incoming' | 'recieved' | 'outgoing' | 'dispatch' | 'completed'

// Helper function to determine document status
const getDashboardStatus = (doc: any): DashboardStatus => {
    const docStatus = doc.status.toLowerCase()
    const transitStatus = doc.transit_status?.toLowerCase()

    // Transit status takes precedence if it exists
    if (transitStatus && transitStatus in statusMapping) {
        return statusMapping[transitStatus as DocumentStatus] as DashboardStatus
    }

    // Fall back to document status
    return statusMapping[docStatus as DocumentStatus] as DashboardStatus || 'dispatch'
}
    ...
    const chartData = useMemo(() => {
        ...
        const weeklyData = daysOfWeek.map(() => {
            ...
            const dayDocs = docs.filter(
                (doc) => doc.date_created && doc.date_created.split('T')[0] === dateStr
            )
